...
datax = pd.read_csv(f"{config['input_dir']}/emb/emb_Z0_825.csv")
...
hie_loinc_rxn_phe = pd.read_csv(f'{config["input_dir"]}/Hierarchy/hie_loinc_rxn_phe_9_2.csv')
...
REL_pairs = pd.read_csv(f'{config["input_dir"]}/similar_related_pairs/related_pairs_0917.csv')
...
# edges = pd.read_csv(f'{config["input_dir"]}/adj/adj_train_9_17.csv')  # (63413,2)
...
features_torch = torch.from_numpy(datax.iloc[:, 1:1537].to_numpy())
