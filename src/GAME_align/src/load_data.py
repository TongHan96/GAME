...
mgb_emb, va_emb, upmc_emb = torch.load(f'{config["path"]}/input/inst_emb.pth')
...
edges = torch.tensor(np.load(f"{config['path']}/input/edges_1229.npy", allow_pickle=True))
edges_rel = torch.tensor(np.load(f"{config['path']}/input/edges_rel_1229.npy", allow_pickle=True))
edges_sim = torch.tensor(np.load(f"{config['path']}/input/edges_sim_1229.npy", allow_pickle=True))

same_desc_edge = torch.tensor(np.load(f"{config['path']}/input/same_desc_edges.npy", allow_pickle=True))
edges_sppmi = torch.tensor(np.load(f"{config['path']}/input/edges_sppmi_1229.npy", allow_pickle=True))
pos_sppmi = torch.tensor(np.load(f"{config['path']}/input/pos_sppmi_1229.npy", allow_pickle=True))
neg_sppmi = torch.tensor(np.load(f"{config['path']}/input/neg_sppmi_1229.npy", allow_pickle=True))
...
emb_sap = pd.DataFrame(sap_emb.numpy())
emb_coder = pd.read_csv(f"{config['input_dir']}/emb/coder_emb.csv", index_col=0)

emb_svd_MGB = pd.DataFrame(mgb_emb.numpy())
emb_svd_VA = pd.DataFrame(va_emb.numpy())
emb_svd_UP = pd.DataFrame(upmc_emb.numpy())

# index in unique_name
if config['path_origin'] == 'align_NA':
    loaded_data = np.load(f"{config['path']}/input/inst_index.npy", allow_pickle=True)
    config['mgb_row'] = loaded_data.item()['mgb_index']
    config['va_row'] = loaded_data.item()['va_index']
    config['upmc_row'] = loaded_data.item()['up_index']
    set_config(config)