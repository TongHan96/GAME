...
def save_hyperparameters(config, start_time, UPLOAD=False):
    ...
    if UPLOAD:
        s3 = boto3.client("s3")
        with open(f"{config['path']}/output/{start_time}/hyper_par.csv", "rb") as f:
            s3.upload_fileobj(f, "han-result", f"{config['path']}/output/{start_time}/{filename}")
...
def write_file_sub(MGB_AUC1, name, UPLOAD=False):
    ...
    with open(f"{config['path']}/output/{start_time}/{name}.csv", "a") as aucfile:
        ...
        aucfile.close()
    if UPLOAD:
        with open(f"{config['path']}/output/{start_time}/{name}.csv", "rb")  as f:
...
def write_file(Epoch, Batch, loss=None, loss2=None, pre=None, pre0=None, pre_latent=None, 
    ...
    # print(LATENT)
    if begin:
        if not os.path.exists(f"{config['path']}/output/{start_time}"):
        ...
        lossfile.close()
        if UPLOAD:
            with open(f"{config['path']}/output/{start_time}/ALL_LOSS.csv", "rb") as f:
        ...
        if UPLOAD:
            with open(f"{config['path']}/output/{start_time}/ALL_PRE.csv", "rb")  as f:
                s3.upload_fileobj(f, "han-result", f"{start_time}/ALL_PRE.csv")
        
        prefile = open(f"{config['path']}/output/{start_time}/ALL_PRE0.csv", "a")
        prewriter = csv.writer(prefile)
        prewriter.writerow(["EPOCH", "BATCH", "TOP1", "TOP5", "TOP10", "TOP20"])
        prefile.close()
        
        if UPLOAD:
            with open(f"{config['path']}/output/{start_time}/ALL_PRE0.csv", "rb")  as f:
                s3.upload_fileobj(f, "han-result", f"{start_time}/ALL_PRE0.csv")
                
        if LATENT:
            prefile = open(f"{config['path']}/output/{start_time}/ALL_PRE_LATENT.csv", "a")
            prewriter = csv.writer(prefile)
            prewriter.writerow(["EPOCH", "BATCH", "TOP1", "TOP5", "TOP10", "TOP20"])
            prefile.close() 

            if UPLOAD:
                with open(f"{config['path']}/output/{start_time}/ALL_PRE_LATENT.csv", "rb")  as f:
                    s3.upload_fileobj(f, "han-result", f"{start_time}/ALL_PRE_LATENT.csv")
            
            lossfile =  open(f"{config['path']}/output/{start_time}/ALL_LOSS2.csv", "a")
            losswriter = csv.writer(lossfile)
            ...
            if UPLOAD:
                with open(f"{config['path']}/output/{start_time}/ALL_LOSS2.csv", "rb") as f:
    ...
    if loss2 is not None:
        lossfile = open(f"{config['path']}/output/{start_time}/ALL_LOSS2.csv", "a", newline="")
        losswriter = csv.writer(lossfile)
    ...
    if pre is not None:
        ...
        if UPLOAD:
            with open(f"{config['path']}/output/{start_time}/ALL_PRE.csv", "rb")  as f:
                s3.upload_fileobj(f, "han-result", f"{config['path']}/output/{start_time}/ALL_PRE.csv")
                
    if pre0 is not None:
        pre0file = open(f"{config['path']}/output/{start_time}/ALL_PRE0.csv", "a")
        ...
        if UPLOAD:
            with open(f"{config['path']}/output/{start_time}/ALL_PRE0.csv", "rb")  as f:
                s3.upload_fileobj(f, "han-result", f"{config['path']}/output/{start_time}/ALL_PR0E.csv")
    ...
    if pre_latent is not None:
        ...
        if UPLOAD:
            with open(f"{config['path']}/output/{start_time}/ALL_PRE_LATENT.csv", "rb") as f:
                s3.upload_fileobj(f, "han-result", f"{config['path']}/output/{start_time}/ALL_PRE_LATENT.csv")
