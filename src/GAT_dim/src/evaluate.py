...
def test(x1, name_all, related_pairs, similar_pairs, 
         PRE=True, AUC=True, rmax=config['rmax'], AUC_type=True, LEVEL=[1], LATENT=False):
    
    # x_low_dim is used to do similar jobs
    x_low_dim = x1[:, 0:rmax]
    results = []
    ...
    if AUC:
        if LATENT:
            results.append(compute_auc(x1, related_pairs, True))
            results.append(compute_auc(x_low_dim, similar_pairs, True))
        else:
            RELA_MGB_AUC, RELA_VA_AUC, RELA_UP_AUC = compute_auc(x1, related_pairs, False)
