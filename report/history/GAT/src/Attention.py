...
class GATLayer_A(nn.Module):
    ...
    def __init__(self, in_features, out_features, K, low_dim=0, residual=False):
        ...
        if low_dim:
            self.V = nn.ParameterList([nn.Parameter(torch.Tensor(in_features, low_dim)) for _ in range(K)])
        else:
            self.A = nn.Parameter(torch.Tensor(in_features, in_features))
    ...
    def forward(self, h, edge_index):
        ...
        if self.LOW_DIM:
            for k in range(self.K):
                h_proj = h @ self.V[k] @ self.V[k].transpose(0,1) # [num_nodes, in_features]
                row, col = edge_index
                alpha = (h_proj[row] * h[col]).sum(dim=-1, keepdim=True)  # [num_edges, 1]
...
#     :param low_dim is dimension of V if not 0
#     '''
#     def __init__(self, in_features, out_features, K, low_dim=0):
#         super(GATLayer_A, self).__init__()
#         self.in_features = in_features
#         self.out_features = out_features
#         self.K = K
...
#                 nn.init.xavier_uniform_(self.V[k].data)
#             else:
#                 nn.init.xavier_uniform_(self.A[k].data)
...
#         super(GAT_A, self).__init__()
#         self.gat1 = GATLayer_A(in_features, hidden_features, K, low_dim)
#         self.gat2 = GATLayer_A(hidden_features * K, hidden_features, K, low_dim)
#         self.linear = nn.Linear(K * hidden_features, out_features)
    
#     def forward(self, x, edge_index):
#         x, alpha1 = self.gat1(x, edge_index)
#         x = F.relu(x)
#         edge_index_new = dropout_adj(edge_index, p=config['drop_p'], force_undirected=True)[0]
#         x, alpha2 = self.gat2(x, edge_index_new)
#         x = F.relu(x)
#         edge_index_new = dropout_adj(edge_index, p=config['drop_p'], force_undirected=True)[0]
#         x = self.linear(x)
#         x = x / torch.norm(x, dim=1, keepdim=True)
...
#         self.gat_conv = GATConv(in_features, out_features, heads, concat=concat, add_self_loops=False)
...
class GATLayer(nn.Module):
    def __init__(self, in_features, out_features, heads, concat, dropout, residual=False):
        ...
        if self.residual:
            ...
            # nn.init.zeros_(self.res_connection.weight)
            nn.init.xavier_uniform_(self.res_connection.weight)
...
#     def __init__(self, in_features, hidden_features, dropout=config['drop_p']):
#         super(GATModel, self).__init__()
#         self.gat1 = GATLayer(in_features, hidden_features, 2, True, dropout) 
#         self.gat2 = GATLayer(2 * hidden_features, 1 * hidden_features, 3, True, dropout)
#         self.linear = nn.Linear(3 * hidden_features, config['out_dim']) 
    
#     def forward(self, x, edge_index):
#         x, attention1 = self.gat1(x, edge_index)
#         x, attention2 = self.gat2(x, edge_index)
#         x = self.linear(x)
#         x = x / torch.norm(x, dim=1, keepdim=True)
...
class GATModel(nn.Module):
    def __init__(self, in_features, hidden_features, dropout=config['drop_p'], out_dim=config['out_dim']):
        super(GATModel, self).__init__()
        self.gat1 = GATLayer(in_features, hidden_features, 2, True, dropout, residual=True)
        self.gat2 = GATLayer(2 * hidden_features, 1 * hidden_features, 3, True, dropout, residual=True)
        self.linear = nn.Linear(3 * hidden_features, out_dim) 
...
Loss Functions
'''
def custom_loss(my_objects, x, now_index, device, name_all, COS_origin_sap=None, COS_origin_svd=None, scale_one_one=config['scale_one_one'], 
                scale_hie=config['scale_hie'], scale_OTOL=config['scale_OTOL'], scale_LTOL=config['scale_LTOL'], scale_REL=config['scale_REL'],
                scale_SIM_NO_HIE=config['scale_SIM_NO_HIE'], scale_LSAP=config['scale_LSAP'], scale_LSVD=config['scale_LSVD'], ORIGIN=True):  
    '''
    Return 8 parts loss
    1. one-one loss
    2. hierarchy loss
    ...
    def calculate_loss(loss_type, my_objects, x, now_index, name_all, scale, DIFF=True,  AA=config['AA'], BB=config['BB'], lambd=config['lambd'], ORIGIN=True):
        ...
        if loss_type == 'one_one':
            ...
            set1 = [set(my_objects[i].rel) if isinstance(my_objects[i].rel, np.ndarray) else my_objects[i].rel for i in now_index]
            ...
            raise ValueError("Invalid loss_type. Supported values are 'one_one', 'hierarchy', 'other_to_loinc', 'local_to_loinc', 'related_pairs' and 'similar_no_hie_pairs'.")
    ...
    P_LOSS_REL, N_LOSS_REL = calculate_loss('related_pairs', my_objects, x, now_index, name_all, scale_REL, ORIGIN=ORIGIN)
    ...
    # Call the function to compute last 2 types of losses
