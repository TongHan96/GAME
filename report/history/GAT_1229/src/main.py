...
def update_config_from_args():
    ...
    parser.add_argument("--scale_sppmi", type=float, default=1,
                        help="Parameter scale_sppmi. Default: 1.")
...
def main(config): 
    # load data
    from load_data import mgb_emb, va_emb, upmc_emb, unique_name, val_rel_pairs, ALL_sim_val_pairs, edges_sppmi, my_objects, pos_sppmi, neg_sppmi, edges, edges_sim, edges_rel
    ...
    if config['path_origin'] is not None:
        ...
        x_sim = torch.load(f"{config['path']}/output/{config['path_origin']}/sim_emb.pth", map_location=device)
        x_detached = x_sim.detach()
        x_detached = x_detached.to(device)
        pos_sppmi = torch.tensor(pos_sppmi).to(device)
        neg_sppmi = torch.tensor(neg_sppmi).to(device)
        edges_sppmi = torch.tensor(edges_sppmi).to(device)
        edges_rel = torch.tensor(edges_rel).to(device)

    optimizer0 = optim.SGD(model_all.parameters(), lr=config['base_lr'])
    scheduler0 = CustomExponentialLR(optimizer0, gamma=config['gamma'], min_lr=1e-6)
    
    if config['path_origin'] is None:
        edge_index = torch.cat((torch.tensor(edges), torch.tensor(edges_sim)), dim=1)
    else:
        edge_index = torch.cat((torch.cat((edges_rel, edges_sppmi), dim=1), pos_sppmi), dim=1).to(device)
    ...
    for epoch in range(1, config['epochs']+1):
        ...
        if config['path_origin'] is None:
            ...
            if epoch % 10 == 1:
                model_all.eval()
                x_sim_test = model_all(mgb_emb, va_emb, upmc_emb, undirected_edge_index)
                # evaluate
            ...
            if epoch % 10 == 1:
                # evaluate
                model_all.eval()
                x_rel_part_test = model_all(mgb_emb, va_emb, upmc_emb, undirected_edge_index)
                x_rel_test = torch.cat((x_detached, x_rel_part_test), dim=1)
                AUC_new = test(x_rel_test, unique_name, config, related_pairs=val_rel_pairs, PRE=False, AUC=True, AUC_type=True)               
                # write
                write_file(epoch, 0, config, REL_AUC=AUC_new[0][0][0])
