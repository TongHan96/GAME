...
datax <- read.csv(paste0('https://han-attention.s3.amazonaws.com/input/drug_side_concat/concat_',now_method,'.csv'))
print(paste("origin embedding dimension:", dim(datax[,2:ncol(datax)])))
name_all <- read.csv('https://han-attention.s3.amazonaws.com/input/name_desc/unique_name_desc.csv')
tmp = strsplit(name_all[,1], split=':')
type = sapply(tmp, function(x) x[1] )

index = c(which(type == "RXNORM"), which(type == "PheCode"))
name_all = name_all[,1]
name_all = name_all[index]

datax = datax[index,2:ncol(datax)]
datax <- t(apply(datax, 1, as.numeric))
...
filename <- paste0("output/AUC_drug_side_concat_", now_method, now_time)
ALL = supervise_MGB(pairs_all, name_all, datax, MGB_index=1:length(index), coef=coef, scale=c(0.01,0.01), maxstep = 2000, stepsize = now_step, epsilon = 1e-2)